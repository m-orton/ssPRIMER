                                          ...
                                          tabsetPanel(id="panels",     
                                                      ...
                                                      tabPanel("Primary Constraints", id='panel3a', value="panel3a",
                                                               ...
                                                               sliderInput("inSlider13", "Min Target Hybridization Efficiency (minimum fraction 
                                                                           ...
                                                                           set each PCR cycle, ***Very high hybridization efficiency of 99-100 will lower 
                                    ...
                                    sidebarPanel(
                                      ...
                                      div(id="about1",
                                          ...
                                          p("Author of tool: Matthew G. Orton, morton01@uoguelph.ca, 
                                             With contributions from Dr. Sally J. Adamowicz, Kamil Chatila-Amos, Alexandra Albin and Samantha Majoros. 
                                             Please contact us at my email for any questions or feedback."), 
                                          ...
                                          p("https://bioconductor.org/packages/release/bioc/html/DECIPHER.html"),
                                          p("http://bioconductor.org/packages/release/bioc/html/Biostrings.html"),
                                          p("https://cran.r-project.org/web/packages/TmCalculator/index.html"),
                                          p("http://unafold.rna.albany.edu/?q=DINAMelt/OligoArrayAux"),
                                          ...
                                          p("Pagès H, Aboyoun P, Gentleman R, DebRoy S (2019). Biostrings: Efficient manipulation of biological strings. R package version 2.52.0."),
                                          
                                          tags$hr(),
                                          
                                          p("Citations for OligoArrayAux:"), 
                                          p("Markham, N. R. & Zuker, M. (2005) DINAMelt web server for nucleic acid melting prediction. Nucleic Acids Res., 33, W577-W581."),
                                          p("Markham, N. R. & Zuker, M. (2008) UNAFold: software for nucleic acid folding and hybridization. In Keith, J. M., editor, Bioinformatics, Volume II. Structure, 
                                            Function and Applications, number 453 in Methods in Molecular Biology, chapter 1, pages 3–31. Humana Press, Totowa, NJ. ISBN 978-1-60327-428-9.")
...
server <- function(input, output, session){
  ...
  observeEvent(input$run3a, {
    ...
    withProgress(message = "Starting Design Process...", value = 0, {
      ...
      primers <- DesignPrimers(tiles, identifier=reactTarget$target, start=primerVar$startPos, end=primerVar$endPos,
                               ...
                               numPrimerSets=20, minProductSize=primerVar$minProductSize, maxProductSize=primerVar$maxProductSize, 
                               primerDimer=primerVar$dimers)
      ...
      # Three different tileseqs at different probe lengths within probe length range set by user
      ...
      primerprobes$startPosFP <- as.numeric(primerprobes$start_forward) + 1
      primerprobes$endPosFP <- as.numeric(primerprobes$start_forward) + as.numeric(primerprobes$length_FP)
      ...
      primerprobes$startPosRP <- as.numeric(primerprobes$start_reverse) - as.numeric(primerprobes$length_RP) - 2
      primerprobes$endPosRP <- as.numeric(primerprobes$start_reverse) + 1
      ...
      dfPrimers$primerGraphTab <- (dfPrimers$primerGraphTab[,c("setID","targetCoverage", "ampliconLength", "gcFP", "gcRP", "gcProbe", "FP_hybr_efficiency", 
                                                               "RP_hybr_efficiency", "hybrEfficiencyAverage", "annealAveragePrimer", "annealDiffprimer", 
                                                               "annealProbe", "annealDiffProbe")])
      
      updateSelectInput(session, "inSelectPrimer2", choices = colnames(dfPrimers$primerGraphTab), selected = "targetCoverage")
  ...
  observeEvent(input$inSelectPrimer2, {
    ...
    dfPrimers$primerGraphTab$ampliconLength <- as.numeric(dfPrimers$primerGraphTab$ampliconLength)
    dfPrimers$primerGraphTab$targetCoverage <- as.numeric(dfPrimers$primerGraphTab$targetCoverage)
    dfPrimers$primerGraphTab$gcFP <- as.numeric(dfPrimers$primerGraphTab$gcFP)
    dfPrimers$primerGraphTab$gcRP <- as.numeric(dfPrimers$primerGraphTab$gcRP)
    dfPrimers$primerGraphTab$gcProbe <- as.numeric(dfPrimers$primerGraphTab$gcProbe)
    dfPrimers$primerGraphTab$FP_hybr_efficiency <- as.numeric(dfPrimers$primerGraphTab$FP_hybr_efficiency)
    dfPrimers$primerGraphTab$RP_hybr_efficiency <- as.numeric(dfPrimers$primerGraphTab$RP_hybr_efficiency)
    dfPrimers$primerGraphTab$hybrEfficiencyAverage <- as.numeric(dfPrimers$primerGraphTab$hybrEfficiencyAverage)
    dfPrimers$primerGraphTab$annealAveragePrimer <- as.numeric(dfPrimers$primerGraphTab$annealAveragePrimer)
    dfPrimers$primerGraphTab$annealDiffprimer <- as.numeric(dfPrimers$primerGraphTab$annealDiffprimer)
    dfPrimers$primerGraphTab$annealDiffProbe <- as.numeric(dfPrimers$primerGraphTab$annealDiffProbe)
    dfPrimers$primerGraphTab$annealProbe <- as.numeric(dfPrimers$primerGraphTab$annealProbe)
    
    output$plot <- renderPlot({
      ggplot(dfPrimers$primerGraphTab, aes_string(x="setID", y=input$inSelectPrimer2, fill="setID")) + geom_bar(stat="identity", width=0.35) +
        theme(axis.title=element_text(size=15), legend.title = element_blank()) + 
